...
export default function SubscribeNow() {
  ...
  const [subs, setSubs] = useState<{ product: Product; qty: number }[]>([]);
  ...
  const addToSubs = (product: Product) => {
    const existing = subs.find(item => item.product._id === product._id);
    if (existing) {
      existing.qty += 1;
    } else {
      setSubs([...subs, { product, qty: 1 }]);
    }
  };
  ...
  const removeItem = (id: string) => {
    setSubs(subs.filter(item => item.product._id !== id));
  };

  const totalAmount = subs.reduce((sum, item) => sum + item.product.price * item.qty, 0);
  ...
  const generateWhatsAppMessage = () => {
    ...
    message += `\n*Total: Rs ${totalAmount}*\n\nPlease confirm delivery address and start date.`;
  ...
  return (
    <section className="py-16 px-4 max-w-6xl mx-auto">
      <h2 className="text-3xl font-bold text-center mb-8">Subscribe to Products</h2>
        ...
        {['daily', 'weekly', 'monthly'].map(freq => (
          <button
            ...
            className={`px-4 py-2 rounded font-semibold ${
              frequency === freq ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300'
            }`}
          >
        ...
        <div className="bg-gray-900 p-6 rounded-lg shadow-lg">
          ...
          {confirmed && (
            <div className="mt-6 text-green-400 font-semibold text-center">
              ✅ Subscription request sent via WhatsApp. Awaiting payment confirmation.
            </div>
          )}
        </div>
      )}
